"use client";
import { UserSubCardItems } from "@/lib/companycard/cards";
import Link from "next/link";

export default function UserSubCompanyCard({ params }) {
  // Generate dynamic route based on company ID
  const getRoute = (id) => `/dashboard/user/subCompanies/${id}`;

  return (
    <div className="grid lg:grid-cols-3 gap-5 lg:gap-8 pb-10 place-items-center ">
      {UserSubCardItems.map((company) => (
        <Link
